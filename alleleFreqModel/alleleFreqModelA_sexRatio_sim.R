...
#get real frequency of each allele in 1990 (same total # of alleles in males & females)
...
#save
save(simdataTrue,file='working_files/intermediate_files/simdataTrueA_FSR.rdata')
...
countsAll<-ddply(indivlist,.(Year,Sex),summarize,genotyped=2*sum(Genotyped=='Y'),
                 total=2*length(Category))
...
save(sim,file=paste("working_files/intermediate_files/SimAlleleFreqAYr_FSR_",year,".rdata",sep=''))
...
for(year in c(1999:2013))
{
  ...
  moms_of_daughters_geno<-merge(moms_of_daughters,simdataTrueUnique[,c(1,8:(nloci+7))],by.x='Indiv',by.y='Indiv',
                           all.x=TRUE)
  #collect simulated moms of sons genotypes (sampled based on real genotyping status) from simdataSampleUnique
  ...
  sim<-foreach(i=names(simdataTrue)[8:(nloci+7)],.combine=cbind) %dopar% {
    ...
    tmp<-data.frame(Year=rep(year,each=74),Category=c('pMt','xMt','errMT','pMs','xMs',
                                                      ...
                                                      'pFb-pFt','xFb-xFt','errFB-errFT','pFdad','xFdad','errFdad', 'pFmom','xFmom','errFmom', #include pFmom, xFmom, errFmom for autosomes
    ...
    #We now need 'pFmom','xFmom','errFmom' since this is autosomes
    ...
    tmp[frqYr1==year & frqCat1=='xFt',3]<-
      mean(simdataSample[simdataSample$Year==year & simdataSample$Sex==2,i])/2
  ...
  simName<-data.frame(Year=rep(year,each=74),Category=c('pMt','xMt','errMT','pMs','xMs',
                                                        'errMS','pMi','xMi','errMI','pMb','xMb','errMB','pMt1-pMt','pMs-pMt',
                                                        'xMs-xMt','errMS-errMT','pMi-pMt','xMi-xMt','errMI-errMT','pMb-pMt','xMb-xMt',
                                                        'errMB-errMT','pMdad','xMdad','errMdad','pMmom','xMmom','errMmom','pMfam','pMmend',
                                                        'pMfam-pMt','xMfam','xMmend','xMfam-xMt','errMFAM','errMMEND','errMFAM-errMT', 
                                                        'pFt','xFt','errFT','pFs','xFs','errFS','pFi','xFi','errFI','pFb','xFb','errFB',
                                                        'pFt1-pFt','pFs-pFt','xFs-xFt','errFS-errFT','pFi-pFt','xFi-xFt','errFI-errFT',
                                                        'pFb-pFt','xFb-xFt','errFB-errFT','pFdad','xFdad','errFdad','pFmom','xFmom','errFmom', #include pFmom, xFmom, errFmom
                                                        'pFfam','pFmend','pFfam-pFt','xFfam','xFmend','xFfam-xFt','errFFAM','errFMEND',
                                                        'errFFAM-errFT'),stringsAsFactors=FALSE)
...
save(simAlleleFreq,file=paste("working_files/intermediate_files/simAlleleFreqA_FSR.rdata",sep=''))
...
#save output
save(simVar,file=paste("working_files/intermediate_files/simVarA_FSR.rdata",sep=''))
