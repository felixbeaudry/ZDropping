...
#A SNP info
map<-read.table('~/Downloads/genedropZplinkInput/FSJfullPedFiltDogFINAL12July2016finalNumCoreDemoMAF05_V2positions.map')
ASNPS <- map[map$V1 %in% c(0:38),]
names(ASNPS) <- c('Chr', 'SNP', 'genMap', 'SNPpos')
ASNPS$SNP_name <- names(indivlistgenoA[,-c(1:7)])
ASNPS <- ASNPS[ASNPS$Chr %in% c(1:38),]
...
for(lg in unique(sizes$V1)){
  ...
  #loop across windows
  for(win in seq(from=0,to=max(ASNPS_chip$SNPpos[ASNPS_chip$scaffold == lg],na.rm=T),by = w_size)){
   if(win+w_size<size_tmp){
    cat(lg," ", ((win/w_size) + win_global)," ",win," ",size_tmp,"\n")
     ASNPS_chip$bootstrap[ASNPS_chip$SNPpos > win & ASNPS_chip$SNPpos <= (win+w_size) & ASNPS_chip$scaffold == lg] <- (win/w_size) + win_global
...
markersInPedOrder_chip_A <- ASNPS_chip[,c('SNP_name','scaffold','bootstrap')]
...
today<-format(Sys.Date(),format="%d%b%Y")
save(allVar,file=paste("allVar_boot_A_w",(w_size/1000000),"mb_",today,".rdata",sep=''))
