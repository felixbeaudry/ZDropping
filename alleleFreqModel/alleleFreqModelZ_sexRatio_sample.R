...
for(year in c(1999:2013))
{
  ...
  #  sampleFreq[SNPyr==(year-1) & SNPcat=='xt',c(4:252)]
	...
	sampleFreq[SNPyr==year & SNPcat=='xMt1-xMt',c(3:(snp_length+2))]<-
		sampleFreq[SNPyr==year & SNPcat=='xMt',c(3:(snp_length+2))]-
		sampleFreq[SNPyr==(year-1) & SNPcat=='xMt',c(3:(snp_length+2))]
	sampleFreq[SNPyr==year & SNPcat=='xFt1-xFt',c(3:(snp_length+2))]<-
	  sampleFreq[SNPyr==year & SNPcat=='xFt',c(3:(snp_length+2))]-
	  sampleFreq[SNPyr==(year-1) & SNPcat=='xFt',c(3:(snp_length+2))]
	
	sampleFreq[SNPyr==year & SNPcat=='xMs-xMt',c(3:(snp_length+2))]<-
		sampleFreq[SNPyr==year & SNPcat=='xMs',c(3:(snp_length+2))]-
		sampleFreq[SNPyr==(year-1) & SNPcat=='xMt',c(3:(snp_length+2))]
	sampleFreq[SNPyr==year & SNPcat=='xFs-xFt',c(3:(snp_length+2))]<-
	  sampleFreq[SNPyr==year & SNPcat=='xFs',c(3:(snp_length+2))]-
	  sampleFreq[SNPyr==(year-1) & SNPcat=='xFt',c(3:(snp_length+2))]
	
	sampleFreq[SNPyr==year & SNPcat=='xMi-xMt',c(3:(snp_length+2))]<-
		sampleFreq[SNPyr==year & SNPcat=='xMi',c(3:(snp_length+2))]-
		sampleFreq[SNPyr==(year-1) & SNPcat=='xMt',c(3:(snp_length+2))]
	sampleFreq[SNPyr==year & SNPcat=='xFi-xFt',c(3:(snp_length+2))]<-
	  sampleFreq[SNPyr==year & SNPcat=='xFi',c(3:(snp_length+2))]-
	  sampleFreq[SNPyr==(year-1) & SNPcat=='xFt',c(3:(snp_length+2))]
	
	sampleFreq[SNPyr==year & SNPcat=='xMb-xMt',c(3:(snp_length+2))]<-
		sampleFreq[SNPyr==year & SNPcat=='xMb',c(3:(snp_length+2))]-
		sampleFreq[SNPyr==(year-1) & SNPcat=='xMt',c(3:(snp_length+2))]
	sampleFreq[SNPyr==year & SNPcat=='xFb-xFt',c(3:(snp_length+2))]<-
...
for(year in c(1999:2013))
{
  ...
  #  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xt1-xt',c(4:252)])^2)
	...
	sampleVar[varYr==year & varCat=='xMt1-xMt','avg']<-
		mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMt1-xMt',c(3:(snp_length+2))])^2)
	sampleVar[varYr==year & varCat=='xFt1-xFt','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFt1-xFt',c(3:(snp_length+2))])^2)
		
	sampleVar[varYr==year & varCat=='xMs-xMt','avg']<-
		mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMs-xMt',c(3:(snp_length+2))])^2)
	sampleVar[varYr==year & varCat=='xFs-xFt','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFs-xFt',c(3:(snp_length+2))])^2)
	
	sampleVar[varYr==year & varCat=='xMi-xMt','avg']<-
		mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMi-xMt',c(3:(snp_length+2))])^2)
	sampleVar[varYr==year & varCat=='xFi-xFt','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFi-xFt',c(3:(snp_length+2))])^2)
	
	sampleVar[varYr==year & varCat=='xMb-xMt','avg']<-
		mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMb-xMt',c(3:(snp_length+2))])^2)
	sampleVar[varYr==year & varCat=='xFb-xFt','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFb-xFt',c(3:(snp_length+2))])^2)

	#covar
	sampleVar[varYr==year & varCat=='xMsxMi','avg']<-
		mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMs-xMt',c(3:(snp_length+2))])*
		as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMi-xMt',c(3:(snp_length+2))]))
	sampleVar[varYr==year & varCat=='xFsxFi','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFs-xFt',c(3:(snp_length+2))])*
	  as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFi-xFt',c(3:(snp_length+2))]))
		
	sampleVar[varYr==year & varCat=='xMsxMb','avg']<-
		mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMs-xMt',c(3:(snp_length+2))])*
		as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMb-xMt',c(3:(snp_length+2))]))
	sampleVar[varYr==year & varCat=='xFsxFb','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFs-xFt',c(3:(snp_length+2))])*
	  as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFb-xFt',c(3:(snp_length+2))]))
		
	sampleVar[varYr==year & varCat=='xMixMb','avg']<-
		mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMi-xMt',c(3:(snp_length+2))])*
		as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMb-xMt',c(3:(snp_length+2))]))
	sampleVar[varYr==year & varCat=='xFixFb','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFi-xFt',c(3:(snp_length+2))])*
	  as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFb-xFt',c(3:(snp_length+2))]))
	
	sampleVar[varYr==year & varCat=='xMsxFs','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMs-xMt',c(3:(snp_length+2))])*
	         as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFs-xFt',c(3:(snp_length+2))]))
	sampleVar[varYr==year & varCat=='xMixFi','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMi-xMt',c(3:(snp_length+2))])*
	         as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFi-xFt',c(3:(snp_length+2))]))
	sampleVar[varYr==year & varCat=='xMbxFb','avg']<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMb-xMt',c(3:(snp_length+2))])*
	         as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFb-xFt',c(3:(snp_length+2))]))
...
for(year in c(1999:2013))
{
	...
	dadsofmalesgeno<-merge(dadsofmales,genoUnique[,c(2,8:(snp_length+7))],by='Indiv',all.x=TRUE)
	...
	momsofmalesgeno<-merge(momsofmales,genoUnique[,c(2,8:(snp_length+7))],by='Indiv',all.x=TRUE)
	...
	dadsoffemalesgeno<-merge(dadsoffemales,genoUnique[,c(2,8:(snp_length+7))],by='Indiv',all.x=TRUE)
...
for(year in c(1999:2013))
{
	...
	sampleFreq[SNPyr==year & SNPcat=='xMfam-xMt',c(3:(snp_length+2))]<-
		sampleFreq[SNPyr==year & SNPcat=='xMfam',c(3:(snp_length+2))]-
		sampleFreq[SNPyr==(year-1) & SNPcat=='xMt',c(3:(snp_length+2))]
	
	sampleFreq[SNPyr==year & SNPcat=='xFfam-xFt',c(3:(snp_length+2))]<-
	  sampleFreq[SNPyr==year & SNPcat=='xFfam',c(3:(snp_length+2))]-
	  sampleFreq[SNPyr==(year-1) & SNPcat=='xFt',c(3:(snp_length+2))]
}
...
for(year in c(1999:2013))
{
	...
	sampleVar[varYr==year & varCat=='xMfam-xMt',3]<-
		mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xMfam-xMt',c(3:(snp_length+2))])^2)
	
	sampleVar[varYr==year & varCat=='xFfam-xFt',3]<-
	  mean(as.numeric(sampleFreq[SNPyr==year & SNPcat=='xFfam-xFt',c(3:(snp_length+2))])^2)
}
...
#save output
save(sampleVar,file=paste("working_files/intermediate_files/sampleVarZ_FSR.rdata",sep=''))
