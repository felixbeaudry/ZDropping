...
#pipeline for estimating expected genetic contributions of 926 individual breeders, assigning unsexed individuals as males or females in a 50:50 ratio
...
do
    python geneDrop_runner_RD.py "$j" working_files/intermediate_files/allABSnestlings.txt "$j".A.0.5 1000000 0 s A 0.5 &
    ((i++))
    [[ $((i%nprocs)) -eq 0 ]] && wait
done
wait
...
for j in working_files/intermediate_files/IndivContrib_*.ped
do
    python geneDrop_runner_RD.py "$j" working_files/intermediate_files/allABSnestlings.txt "$j".Z.0.5 1000000 0 s Z 0.5 &
    ((i++))
    [[ $((i%nprocs)) -eq 0 ]] && wait
done
wait
